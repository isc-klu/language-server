...
function documaticLink(server: ServerSpec, cls: string): string {
	return `[${cls}](${server.scheme}://${server.host}:${server.port}${server.pathPrefix}/csp/documatic/%25CSP.Documatic.cls?LIBRARY=${encodeURIComponent(server.namespace.toUpperCase())}&CLASSNAME=${encodeURIComponent(cls)})`;
}
						...
						contents: {
							...
							value: ["$ZUTIL", "$ZU"].includes(sysftext)
								...
								: markupValue(
										...
										sysfdoc.link
											? `[Online documentation](${sysfdoc.link[0] == "h" ? sysfdoc.link : `https://docs.intersystems.com/irislatest/csp/docbook/Doc.View.cls?KEY=RCOS_${sysfdoc.link}`})`
											: "",
									),
						},
						range: sysfrange,
