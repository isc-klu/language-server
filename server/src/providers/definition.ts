		...
		if (params.position.character >= symbolstart && params.position.character <= symbolend) {
			...
			if (((parsed[params.position.line][i].l == ld.cls_langindex && parsed[params.position.line][i].s == ld.cls_clsname_attrindex) ||
				...
				parsed[params.position.line][i].l == ld.cos_langindex && (
					...
					i == 1 &&
					isClassMember(doc.getText(Range.create(params.position.line, parsed[params.position.line][0].p, params.position.line, parsed[params.position.line][0].p + parsed[params.position.line][0].c)).toLowerCase())
				)) {
				...
				if (parsed[params.position.line][i].s == ld.cos_instvar_attrindex || parsed[params.position.line][i].l == ld.cls_langindex) {
					membercontext = {
						baseclass: thisclass,
						context: ""
					};
