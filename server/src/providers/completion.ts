...
export async function onCompletion(params: CompletionParams): Promise<CompletionItem[] | null> {
	...
	if (prevline.endsWith("$$$") && [ld.cos_langindex, ld.sql_langindex].includes(triggerlang)) {
		...
		const data: QueryData = {
			query: `SELECT Name, Description, Origin, Type, Deprecated FROM %Dictionary.CompiledParameter WHERE Parent = ?${isProperty ? " OR Parent %INLIST (SELECT $LISTFROMSTRING(PropertyClass) FROM %Dictionary.CompiledClass WHERE Name = ?)" : ""}${!(await showInternalForServer(server)) ? " AND Internal = 0" : ""}${!settings.completion.showDeprecated ? " AND Deprecated = 0" : ""}`,
		...
		const data: QueryData = {
			query: `SELECT Name, Description, Origin, RuntimeType, Deprecated FROM %Dictionary.CompiledProperty WHERE Parent = ? AND Calculated = 0${!showInternal ? " AND Internal = 0" : ""}${!settings.completion.showDeprecated ? " AND Deprecated = 0" : ""}`,
			parameters: [thisclass],
		};
		const respdata = await makeRESTRequest("POST", 1, "/action/query", server, data);
		if (Array.isArray(respdata?.data?.result?.content) && respdata.data.result.content.length > 0) {
